...
const transactionType: DataType[] = [
  ...
  },
  {
    ...
    color: 'info',
    percentage: 3.26,
    transactionType: '9 Tháng'
    //src: '/images/cards/firefox.png'
  },
  {
    value: '3.2%',
    transactionType: '12 Tháng',
    color: 'warning',
    percentage: 3.99
...
const SavingAndInterestRate = () => {
  ...
  return (
    <Card sx={{ width: '90%' }}>
      <CardHeader title='Tiền tiết kiệm và lãi suât theo kỳ hạn' />
      <TabContext value={value}>
        ...
        >
          <TableContainer sx={{ width: '100%', minWidth: '800px' }}>
            <Table sx={{ '& th, & td': { border: 0 } }}>
              ...
              <TableBody>
                {activeTabData[value].map((row, index) => (
                  <TableRow key={index} sx={{ '& .MuiTableCell-root': { py: theme => `${theme.spacing(3.75)} !important` } }}>
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Typography sx={{ ml: 3, color: 'text.secondary' }}>
                          {value === 'transactionType' ? row.transactionType : value === 'os' ? row.system : row.country}
                        </Typography>
                      </Box>
                    </TableCell>
                    <TableCell>
...
export default SavingAndInterestRate
